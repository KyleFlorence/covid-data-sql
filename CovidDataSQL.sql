...
-- Filtered location to united states using 'like' and % wildcards for demonstration
Select Location, date, total_cases, population, (total_cases/population)*100 as percent_diagnosed
FROM CovidData..CovidDeaths$
WHERE Location like '%states%'
...
Select Location, MAX(total_cases) as peak_infection_count, population, MAX((total_cases/population))*100 as Infection_rate
FROM CovidData..CovidDeaths$
...
Select Location, MAX(cast(total_deaths as int)) as peak_death_count, population, MAX((total_deaths/population))*100 as Death_rate
FROM CovidData..CovidDeaths$
WHERE continent is NOT NULL
GROUP BY Location, population
...
Select SUM(new_cases) as total_cases, SUM(CAST(new_deaths as int)) as total_deaths, SUM(CAST(new_deaths as int))/SUM(new_cases)*100 as percent_deaths_per_case
FROM CovidData..CovidDeaths$
WHERE continent is NOT NULL
...
Select deaths.location, MAX(CAST(vacc.people_fully_vaccinated as BIGINT)) AS people_fully_vaccinated, MAX(deaths.population) as population
...
With pop_vs_vac (continent, location, date, population, new_vaccinations, rolling_vaccine_count)
as
(
Select deaths.continent, deaths.location, deaths.date, deaths.population, vacc.new_vaccinations
, SUM(CONVERT(BIGINT, vacc.new_vaccinations)) OVER (Partition by deaths.location ORDER BY deaths.location, deaths.date) as rolling_vaccine_count
...
Select *, (rolling_vaccine_count/population)*100 AS rolling_total_per_population
FROM pop_vs_vac
